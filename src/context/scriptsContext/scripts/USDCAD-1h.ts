...
export default (function f({
  ...
  function resistanceBounce() {
    ...
    for (
      let j = horizontalLevelCandleIndex - scriptParams.candlesAmountToBeConsideredHorizontalLevel!.future;
    ...
    if (price > candles[currentDataIndex].close + spread / 2) {
      ...
      const size =
        Math.floor((balance * (scriptParams.riskPercentage / 100)) / (scriptParams.stopLossDistance * 10000 * 0.67)) *
          10000 || 10000;

...
export default function getParamsArray(): ScriptParams[] {
  ...
  for (const stopLossDistance of [45].map((sl) => sl * priceAdjustment)) {
    for (const takeProfitDistance of [330].map((tp) => tp * priceAdjustment)) {
          ...
          for (const trendCandles of [0]) {
            for (const trendDiff of [0].map((td) => td * priceAdjustment)) {
              for (const candlesAmountWithLowerPriceToBeConsideredHorizontalLevel of [10]) {
                for (const priceOffset of [50].map((po) => po * priceAdjustment)) {
                  for (const maxSecondsOpenTrade of [45 * 24 * 60 * 60]) {
                    arr.push({
                      validHours,
                      validDays,
                      validMonths,
